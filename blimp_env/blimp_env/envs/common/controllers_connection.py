...
class ControllersConnection:
    """model controller connection handle"""
    ...
    def switch_controllers(self, controllers_on, controllers_off, strictness=0):
        """
        ...
        try:
            ...
            print(self.switch_service_name + " service call failed" + err)
    ...
    def reset_controllers(self, controllers_reset):
        ...
        result_off_ok = self.switch_controllers(
            controllers_on=[], controllers_off=controllers_reset
        )

        if result_off_ok:
            result_on_ok = self.switch_controllers(
                controllers_on=controllers_reset, controllers_off=[]
            )
