...
def my_create_mlp(
    ...
    for idx in range(len(net_arch) - 1):
        ...
        modules.append(nn.LayerNorm(net_arch[idx + 1]))  # add layer norm
...
class MyQuantileNetwork(QuantileNetwork):
    def __init__(
        self,
        observation_space: gym.spaces.Space,
        action_space: gym.spaces.Space,
    ...
    def forward(self, obs: th.Tensor) -> th.Tensor:
        ...
        # print out quantile value if NAN
        if th.isnan(quantiles).any():
            print("detect NAN in quantile value:")
            print(quantiles)

        return quantiles.view(-1, self.n_quantiles, self.action_space.n)


class MyQRDQNPolicy(QRDQNPolicy):
    def __init__(
        self,
        observation_space: gym.spaces.Space,
        action_space: gym.spaces.Space,
    ...
    def make_quantile_net(self) -> QuantileNetwork:
        ...
        return MyQuantileNetwork(**net_args).to(self.device)


class MyMultiInputPolicy(MyQRDQNPolicy):
    def __init__(
        self,
        observation_space: gym.spaces.Space,
        action_space: gym.spaces.Space,
